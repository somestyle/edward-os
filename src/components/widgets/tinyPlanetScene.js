...
export const STOPS = [
  ...
  { company: 'Kea AI', role: 'Head of Product Design', period: 'Mar 2021 – Jul 2022', year: '2021', kind: 'drivethru' },
...
export function createTinyPlanet(stage, opts) {
  ...
  const car = (m) => { const g = new THREE.Group(); g.add(at(box(0.16, 0.05, 0.08, m), 0, 0.03, 0), at(box(0.09, 0.045, 0.07, M.sky), -0.005, 0.08, 0)); [[-0.05, 0.045], [0.05, 0.045], [-0.05, -0.045], [0.05, -0.045]].forEach(([x, z]) => { const w = at(new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.02, 0.015, 10), M.tire), x, 0.02, z); w.rotation.x = Math.PI / 2; g.add(w); }); return g; };

  const landmark = (kind) => {
    const g = new THREE.Group();
    switch (kind) {
      case 'bank': g.add(box(0.36, 0.05, 0.24, M.stone), at(box(0.4, 0.02, 0.28, M.stone), 0, -0.02, 0)); for (let i = 0; i < 4; i++) g.add(at(cyl(0.02, 0.022, 0.2, M.white, 8), -0.12 + i * 0.08, 0.05, 0.08)); g.add(at(box(0.3, 0.2, 0.14, M.stone), 0, 0.05, -0.03), at(sc(roof4(0.24, 0.1, M.lav), 1, 1, 0.7), 0, 0.3, 0), at(box(0.06, 0.09, 0.012, M.ink), 0, 0.05, 0.043), win(-0.09, 0.19, 0.043), win(0.09, 0.19, 0.043)); break;
      case 'phone': g.add(box(0.16, 0.3, 0.05, M.ink), at(box(0.13, 0.24, 0.02, M.sky), 0, 0.035, 0.02), at(box(0.05, 0.005, 0.02, M.stone), 0, 0.31, 0.02), at(box(0.09, 0.04, 0.01, M.white), 0, 0.2, 0.031), at(box(0.09, 0.02, 0.01, M.coral), 0, 0.14, 0.031), at(cone(0.04, 0.09, M.coral, 12), 0, 0.33, 0), at(ball(0.045, M.coral), 0, 0.42, 0), at(ball(0.016, M.white, 8, 6), 0, 0.42, 0.036)); break;
      case 'newsstand': g.add(box(0.28, 0.2, 0.2, M.mint)); { const r = at(box(0.34, 0.03, 0.26, M.coral), 0, 0.22, 0); r.rotation.x = 0.18; g.add(r); } g.add(at(box(0.22, 0.07, 0.012, M.white), 0, 0.1, 0.105), at(box(0.16, 0.02, 0.06, M.cream), -0.04, 0.2, 0.08), at(box(0.12, 0.02, 0.05, M.white), 0.06, 0.2, 0.07)); { const p = at(cyl(0.03, 0.03, 0.14, M.white, 10), 0.18, 0.03, 0.09); p.rotation.z = Math.PI / 2; g.add(p); } g.add(at(cyl(0.012, 0.012, 0.3, M.trunk, 6), -0.19, 0, -0.05), at(box(0.1, 0.06, 0.008, M.butter), -0.19, 0.24, -0.05)); break;
      ...
      case 'ticker': g.add(box(0.2, 0.42, 0.2, M.stone), at(box(0.22, 0.05, 0.22, M.butter), 0, 0.26, 0)); for (let r = 0; r < 3; r++) for (let c = 0; c < 2; c++) g.add(win(-0.045 + c * 0.09, 0.11 + r * 0.1, 0.105, 0.045, 0.045)); g.add(at(cone(0.06, 0.1, M.lav, 4), 0, 0.42, 0), at(cyl(0.006, 0.006, 0.12, M.ink, 6), 0, 0.5, 0)); break;
      ...
      case 'drivethru': g.add(box(0.26, 0.18, 0.2, M.butter), at(box(0.32, 0.03, 0.26, M.coral), 0, 0.19, 0), win(-0.07, 0.13, 0.105, 0.07, 0.06), at(box(0.08, 0.1, 0.012, M.ink), 0.06, 0.03, 0.105), at(cyl(0.012, 0.012, 0.18, M.stone, 8), 0.2, 0, 0.06), at(box(0.08, 0.1, 0.02, M.ink), 0.2, 0.18, 0.06), at(ball(0.02, M.lamp, 8, 6), 0.2, 0.23, 0.075), at(car(M.red), 0.22, 0, -0.1)); break;
      case 'clinic': g.add(box(0.3, 0.2, 0.22, M.white), at(box(0.34, 0.04, 0.26, M.mint), 0, 0.2, 0), at(box(0.1, 0.03, 0.03, M.red), 0, 0.26, 0), at(box(0.03, 0.1, 0.03, M.red), 0, 0.225, 0), at(box(0.06, 0.09, 0.012, M.sky), 0, 0, 0.115), win(-0.09, 0.15, 0.115), win(0.09, 0.15, 0.115), at(car(M.white), -0.24, 0, 0.02)); break;
      case 'lighthouse': g.add(cyl(0.07, 0.1, 0.44, M.white, 14), at(cyl(0.085, 0.09, 0.07, M.coral, 14), 0, 0.2, 0), at(box(0.04, 0.06, 0.012, M.ink), 0, 0, 0.098), at(box(0.11, 0.09, 0.11, M.lamp), 0, 0.44, 0), at(cone(0.09, 0.08, M.lav, 14), 0, 0.53, 0), at(sc(ball(0.06, M.rockDark, 8, 6), 1.2, 0.6, 1), 0.13, 0.01, 0.06), at(sc(ball(0.045, M.rock, 8, 6), 1, 0.6, 1), -0.11, 0.01, 0.09)); break;
      case 'court': g.add(box(0.36, 0.05, 0.24, M.stone), at(box(0.4, 0.02, 0.28, M.stone), 0, -0.02, 0)); for (let i = 0; i < 4; i++) g.add(at(cyl(0.02, 0.02, 0.22, M.white, 8), -0.12 + i * 0.08, 0.05, 0.08)); g.add(at(box(0.3, 0.22, 0.14, M.white), 0, 0.05, -0.03), at(sc(roof4(0.26, 0.11, M.lilac), 1, 1, 0.7), 0, 0.325, 0), at(ball(0.035, M.gold, 8, 6), 0, 0.42, 0), at(box(0.06, 0.1, 0.012, M.ink), 0, 0.05, 0.043)); break;
      case 'chart': g.add(cyl(0.012, 0.012, 0.18, M.stone, 8), at(cyl(0.06, 0.06, 0.012, M.stone, 12), 0, 0, 0)); { const b = at(box(0.26, 0.19, 0.02, M.white), 0, 0.18, 0); b.rotation.x = -0.35; g.add(b); const ink = M.ink; [[-0.09, 0.27, 0.05, 0], [-0.05, 0.28, 0.03, 0.9], [-0.02, 0.26, 0.03, -0.9], [0.01, 0.29, 0.04, 1.1], [0.045, 0.265, 0.04, -0.7], [0.09, 0.27, 0.05, 0]].forEach(([x, y, w, r]) => { const seg = at(box(w, 0.008, 0.006, M.red), x, y, 0.03); seg.rotation.z = r; seg.rotation.x = -0.35; g.add(seg); }); g.add(at(box(0.05, 0.012, 0.006, ink), 0.09, 0.21, 0.035), at(box(0.012, 0.05, 0.006, ink), 0.09, 0.21, 0.035)); const p1 = at(box(0.035, 0.008, 0.006, M.mint), -0.07, 0.2, 0.035), p2 = at(box(0.05, 0.008, 0.006, M.mint), -0.06, 0.17, 0.038); g.add(p1, p2); } g.add(at(bush(0.6), 0.16, 0, 0.04), at(flowers(M.pink), -0.15, 0, 0.06)); break;
  ...
  TRAIL.forEach((t) => { addPop(roundTree(0.5, M.leafLight), normalAt(t.lat + 6, t.lon + 12)); addPop(bush(0.7), normalAt(t.lat - 6, t.lon - 11)); });
  ...
  const eTex = (() => { const c = document.createElement('canvas'); c.width = c.height = 256; const x = c.getContext('2d'); x.fillStyle = '#fff'; x.font = 'bold 210px ui-rounded, "Arial Rounded MT Bold", "Helvetica Neue", Arial, sans-serif'; x.textAlign = 'center'; x.textBaseline = 'middle'; x.fillText('E', 128, 140); const t = new THREE.CanvasTexture(c); t.anisotropy = 4; return t; })();
  const E = new THREE.Mesh(new THREE.PlaneGeometry(0.07, 0.07), new THREE.MeshBasicMaterial({ map: eTex, transparent: true, toneMapped: false, depthWrite: false }));
  ...
  const settleOn = (i) => { target = targetFor(i); spin.speed = 0; arrived = false; };
